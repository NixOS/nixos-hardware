...
in {
  ...
  systemd.services.fix-touchpad = {
    serviceConfig.ExecStart = ''${pkgs.systemd}/bin/systemd-inhibit --what=sleep --why="fixing touchpad must finish before sleep" --mode=delay ${fix-touchpad}/bin/fix-touchpad.sh'';
