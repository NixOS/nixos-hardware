...
in
{
  ...
  boot = {
    kernelPackages =
      if (lib.versionOlder pkgs.linuxPackages_latest.kernel.version "6.15") then
        lib.mkDefault pkgs.linuxPackages_testing
      else if (lib.versionOlder pkgs.linux.version "6.15") then
